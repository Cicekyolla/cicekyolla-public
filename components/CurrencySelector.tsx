 ...
 *
 * TASARIM: Version 72 / Figma 117 ailesi birebir korunur. Aynı pill yüksekliği,
 * aynı lila tonları (#8B5CF6), aynı caret, aynı focus ring, aynı bottom sheet.
 * Header'da dil seçicinin HEMEN YANINDA durur:  TR ▼   ₺ TRY ▼
 * Yeni koyu/siyah bir tasarım dili GETİRİLMEZ.
 *
 * Panel dil seçicisinden dar (3 seçenek) ve tek sütun; 320px'lik dil paneliyle
 * aynı köşe/gölge/ayraç değerlerini kullanır.
 *
...
export function CurrencySelector() {
  ...
  const panel = open && pos && typeof document !== "undefined" ? createPortal(
    <div
      ...
      className="z-[9999] w-[276px] rounded-[20px] border border-[#8B5CF6]/15 bg-white p-5 pt-6 shadow-[0_8px_40px_rgba(0,0,0,0.10),0_2px_8px_rgba(139,92,246,0.08)]"
    >
      ...
      {source && (
        <p className="mt-3.5 border-t border-[#8B5CF6]/[0.08] pt-3 text-[11px] leading-[1.5] text-[#9CA3AF]">
          {t("currency.rateNote", { source })}
        </p>
      )}
    </div>,
    document.body,
    ...
    <>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        ...
        <SheetContent side="bottom" className="max-h-[88dvh] gap-0 overflow-hidden rounded-t-[28px] border-0 bg-white p-0 shadow-2xl outline-none [&>button:last-child]:hidden">
          ...
          {source && (
            <p className="px-5 pb-[max(16px,env(safe-area-inset-bottom))] pt-3 text-[11px] leading-[1.5] text-[#9CA3AF]">
              {t("currency.rateNote", { source })}
            </p>
          )}
        </SheetContent>
