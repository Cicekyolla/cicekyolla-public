...
export function BlogExperience({posts}:{posts:BlogPost[]}){const cats=["Tümü",...Array.from(new Set(posts.map(p=>p.category)))];const [cat,setCat]=useState("Tümü");const shown=cat==="Tümü"?posts:posts.filter(p=>p.category===cat);const featured=shown[0];return <>
 ...
 <div className="mt-14 flex flex-wrap gap-3">{cats.map(c=><button key={c} onClick={()=>setCat(c)} className={`rounded-full border px-6 py-3 text-sm font-semibold ${cat===c?'border-[#8b5cf6] bg-[#8b5cf6] text-white shadow-lg shadow-violet-200':'border-[#e6dff0] bg-white text-[#6f667b]'}`}><Tag className="mr-2 inline h-3.5 w-3.5"/>{c}</button>)}</div>
 <div className="mt-10 grid gap-7 md:grid-cols-2 lg:grid-cols-3">{shown.map(post=><Link key={post.slug} href={`/blog/${post.slug}`} className="group overflow-hidden rounded-[26px] border border-[#e8e1f0] bg-white shadow-[0_22px_60px_rgba(45,22,72,.07)]"><div className="relative aspect-[16/10] overflow-hidden bg-[#f2eef6]"><img src={post.image} alt={post.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/><span className="absolute left-5 top-5 rounded-full bg-[#8b5cf6] px-4 py-2 text-xs font-bold text-white">{post.category}</span></div><div className="p-6"><div className="flex items-center gap-2 text-xs text-[#a097aa]"><Clock3 className="h-4 w-4"/>6 dk okuma</div><h3 className="mt-4 text-xl font-bold leading-snug">{post.title}</h3><p className="mt-4 line-clamp-3 leading-6 text-[#817889]">{post.excerpt}</p><span className="mt-6 flex items-center gap-2 font-bold text-[#8b5cf6]">Devamını Oku <ArrowUpRight className="h-4 w-4"/></span></div></Link>)}</div></div></main>
 </>}
