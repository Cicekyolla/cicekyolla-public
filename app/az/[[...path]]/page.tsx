// GLOBAL 14-dil — /az/* locale yüzeyi (ince sarmalayıcı; motor lib/global/page).
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  return localeMetadata("az", params.path ?? []);
}

export default function Page({ params, searchParams }: Props) {
  // ?category / ?page yalnız lokasyon kataloğu sayfalaması içindir; metadata (canonical) sorgusuz kalır.
  return <LocalePage locale="az" path={params.path ?? []} searchParams={searchParams} />;
}
