...
//
// Bu rota /istanbul/kartal ve /istanbul/kartal/<mahalle> yollarini karsilar.
...
const toSlug = (rest?: string[]) => ["istanbul", "kartal", ...(rest ?? [])];

export async function generateStaticParams(): Promise<{ rest: string[] }[]> {
  const out: { rest: string[] }[] = [{ rest: [] }];
  try {
    const d = await fetchDistrictNeighborhoods("istanbul", "kartal");
