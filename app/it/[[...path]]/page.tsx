// GLOBAL 14-dil — /it/* locale yüzeyi (ince sarmalayıcı; motor lib/global/page).
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  return localeMetadata("it", params.path ?? []);
}

export default function Page({ params, searchParams }: Props) {
  // ?category / ?page yalnız lokasyon kataloğu sayfalaması içindir; metadata (canonical) sorgusuz kalır.
  return <LocalePage locale="it" path={params.path ?? []} searchParams={searchParams} />;
}
